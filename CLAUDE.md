...
| ドキュメント | 役割 | 更新頻度 |
|---|---|---|
| **CONCEPT.md** | なぜnookが存在するか、何を絶対に譲らないか（不変の原則） | 稀。ここが変わる＝nookの存在意義が変わったということ |
...
CONCEPT.mdの「不変の原則」を追加・削除・変更する場合、あるいはドキュメント間で矛盾する記述（例: 過去に保留にした原則が復活可能かどうか）を見つけた場合は、エージェントが単独で判断せず、grill-meスキル等でユーザーに確認してから反映する。これはnook自体の「Agentは提案するが、承認は人間が行う」という設計思想（CONCEPT.md原則2）をドキュメント作業自体にも適用したもの。
