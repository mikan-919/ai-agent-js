...
- 最終的には`bunx @mikan-919/oriel serve`を環境変数なしで呼べるようにしたい。relay origin、repositoryなど現在env varで渡している利用者固有設定を、初回起動時のWeb UIから入力し、既存のlocal state SQLite（`ORIEL_STATE_PATH`、既定値は issue #47 で導入済み）へ保存する経路に置き換える方向で検討する。
